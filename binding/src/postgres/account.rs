...
impl Db {
    pub async fn get_account_data(&self,uid:i32) -> Result<AccountData,PgCustomError> {
        Ok(sqlx::query_as::<_,AccountData>("SELECT user.id as id, username,password from users where id = $1")
    ...
    pub async fn change_user_password(&self,pass:&str,uid:i32)->Result<(),PgCustomError>{
        ...
        if reg{
            if sqlx::query("Select username from users where username=$1").bind(user).fetch_one(&**self).await.is_ok(){
                return Err(PgCustomError::Custom("The username already used by someone, please use another username".into()));
            }
            ...
            let data = sqlx::query("SELECT id,username,password from users where username=$1").bind(user).fetch_one(&**self).await?;
            if !verify(pass,&data.try_get::<String,_>("password")?).unwrap_or(true){
                return Err(PgCustomError::Custom("Password doesnt match the account".into()));
            }
            uid = AccountData {
    ...
    pub async fn force_psn(&self,psn:&str,uid:i32) -> Result<(),PgCustomError> {
        sqlx::query("update users set psn_id=NULL where psn_id=$1").bind(psn).execute(&**self).await?;
        sqlx::query("update users set psn_id=$1 where id=$2").bind(psn).bind(uid).execute(&**self).await?;
        Ok(())
    }
