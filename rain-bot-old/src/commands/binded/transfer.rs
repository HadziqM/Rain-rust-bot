...
use crate::{ComponentBundle, Components, MyErr, Mybundle, Mytrait, Reg, SlashBundle};
use serenity::all::*;
...
impl<'a> SaveJudge<'a> {
    ...
    async fn save_to_file(&self) -> Result<(), MyErr> {
        ...
        for data in &self.files {
            let name = format!("{}_{}", &self.cmd.user.id.to_string(), data.name.to_owned());
    ...
    let msg = ch
        .send_message(
            ...
            CreateMessage::new()
                ...
                .embed(data.make_embed(bnd.init))
                .components(vec![data.make_button()]),
        )
        .await?;
...
async fn auto_accept(
    ...
    if msg.components.len() != 0 {
        ...
        msg.edit(
            ...
            EditMessage::new()
                .content(format!(
                    "Approved automatically by bot At <t:{}:F>",
                    SystemTime::now()
...
async fn button(bnd: &ComponentBundle<'_>) -> Result<(), MyErr> {
    Components::response(
        bnd,
        "begin lengthy opration, please dont push button for some time",
        true,
    )
    .await?;
    let data: Vec<_> = bnd.cmd.data.custom_id.split("*").collect();
    let check = SaveAcknowladge::check(data)?;
    let user = UserId(NonZeroU64::new(check.uid.parse::<u64>().unwrap()).unwrap())
        .to_user(&bnd.ctx.http)
        .await
        .unwrap();
    if check.accept {
        ...
        user.direct_message(
            ...
            CreateMessage::new().content(&format!(
                "your save file is already approved by {} you can login into the game now",
                bnd.cmd.user.name
            )),
        )
        .await?;
        ...
        user.direct_message(
            &bnd.ctx.http,
            CreateMessage::new().content(&format!(
                "your save file has been rejected by {} you can login into the game now",
                bnd.cmd.user.name
            )),
        )
        .await?;
