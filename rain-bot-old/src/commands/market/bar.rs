...
use crate::{Components, ModalBundle, MyErr, Mybundle, Mytrait, SlashBundle};
use serenity::all::*;

pub(super) async fn slash(bnd: &SlashBundle<'_>) -> Result<(), MyErr> {
    let modal = CreateInteractionResponse::Modal(
        CreateModal::new("bar", "Bar Request Board").components(vec![
...
async fn modal(bnd: &ModalBundle<'_>) -> Result<(), MyErr> {
    ...
    for data in &bnd.cmd.data.components {
        let arow = data
            .components
            .first()
            .ok_or(MyErr::Custom("cant get first component".to_owned()))?;
    ...
    let embed = CreateEmbed::new()
        .title("Bar Request")
        .color(Color::Blue.throw())
        .description(item)
        .field("Requested price", price, false)
        .author(CreateEmbedAuthor::new(&bnd.cmd.user.name).icon_url(bnd.cmd.user.face()));
    Components::response(bnd, "Requested", true).await?;
    let msg = ChannelId::new(bnd.init.log_channel.request_channel)
        .send_message(
            &bnd.ctx.http,
            CreateMessage::new().embed(embed).content(format!(
                "<@&{}> {}",
                bnd.init.server_role.bartender_role,
                bnd.cmd.user.to_string()
            )),
        )
        .await?;
    msg.channel_id
        .create_public_thread(
            &bnd.ctx.http,
            msg.id,
            CreateThread::new(format!("{} Request Thread", &bnd.cmd.user.name)),
        )
        .await?;
    Ok(())
}
