...
use crate::reusable::component::market::{Item, Market, Trading};
use crate::{
    event::interaction::ComponentBundle,
    reusable::{
        bitwise::ItemCode,
        component::{bounty::Title, Mytrait},
    },
    Components, ItemPedia, MyErr, Reg, SlashBundle,
};
...
impl Handle {
    ...
    async fn check(&self, reg: &Reg<'_>, pedia: &ItemPedia) -> Result<Bought, MyErr> {
        ...
        if self.bought as u32 > self.item.treshold {
            return Err(MyErr::Custom(format!(
                "you bought {} item, but market now only had {} item",
                self.bought, self.item.treshold
            )));
        }
        ...
        Ok(Bought {
            item: self.item.item.text(pedia).unwrap(),
            qty: self.bought as i64,
            total,
            change,
            former: bc,
            price: self.item.price as i32,
            unit: "item(s)".to_string(),
            discounted,
            disc,
        })
    }
    async fn transaction(&mut self, reg: &Reg<'_>, pedia: &ItemPedia) -> Result<(), MyErr> {
        ...
        let price = ((self.item.price * self.bought as u32) as f32
            * Title::new(event.title as u8).discount()) as u32;
        let count = self.item.item.count * self.bought;
        self.item.treshold -= self.bought as u32;
        self.market.bought(&self.item);
        let item = ItemCode {
            key: self.item.item.key.to_owned(),
            types: self.item.item.types,
            count,
        };
...
impl Bought {
    ...
    pub fn create_embed(&self, bnd: &SlashBundle<'_>) -> CreateEmbed {
        CreateEmbed::new()
            .title("Receipt")
            .author(CreateEmbedAuthor::new(&bnd.cmd.user.name).icon_url(bnd.cmd.user.face()))
            .description(format!(
                "{} Receipt at <t:{}:F>",
                &bnd.cmd.user,
                crate::reusable::utils::MyTime::now()
            ))
            .field(
                "Item",
                format!("{}\nBought x{} {} ", &self.item, self.qty, &self.unit),
                false,
            )
            .field(
                "Price",
                format!(
                    "Per {}: {} x {} = {}",
                    &self.unit,
                    Market::currency(self.price as i64),
                    self.qty,
                    Market::currency(self.total as i64)
                ),
                false,
            )
            .field(
                ...
                format!(
                    "Discount {} for {} = {}",
                    &self.disc,
                    Market::currency(self.total),
                    Market::currency(self.discounted)
                ),
                false,
            )
            .field(
                ...
                format!(
                    "Your Coin = {} - {} = {}",
                    Market::currency(self.former as i64),
                    Market::currency(self.discounted as i64),
                    Market::currency(self.change as i64)
                ),
                false,
            )
    ...
    async fn wtf(bnd: &SlashBundle<'_>, x: &ComponentInteraction) -> Result<bool, MyErr> {
        if x.user != bnd.cmd.user {
            x.response(
                bnd.ctx,
                Components::interaction_response("the button is not for you", true),
            )
            .await?;
            return Ok(false);
        }
    ...
    bnd.cmd
        .create_response(
            ...
            CreateInteractionResponse::Autocomplete(
                CreateAutocompleteResponse::new().set_choices(market.auto(bnd.pedia, focus)),
            ),
        )
        .await?;
    Ok(())
...
pub async fn slash(bnd: &SlashBundle<'_>, reg: &Reg<'_>) -> Result<(), MyErr> {
    ...
    if !trade.market.enabled {
        return Err(MyErr::Custom(
            "tradimng market is currently disabled".to_string(),
        ));
    }
