                ...
                .filter_map(|x| {
                    ...
                    if name.contains(&foc) || name.starts_with(&foc) {
                        return Some(AutocompleteChoice {
                            ...
                            value: serde_json::Value::Number(x.id.into()),
                        });
                    }
                    None
                })
            ...
            bnd.cmd
                .create_response(
                    ...
                    CreateInteractionResponse::Autocomplete(
                        CreateAutocompleteResponse::new().set_choices(aut),
                    ),
                )
                .await?;
            pg.close().await;
...
impl Guild {
    async fn embed(&self, count: i64, bnd: &SlashBundle<'_>) -> Result<CreateEmbed, MyErr> {
        ...
        if leader.as_str() != "None" {
            let user = UserId::new(self.discord_id.clone().unwrap().parse::<u64>().unwrap())
                .to_user(&bnd.ctx.http)
                .await?;
            emb = emb.footer(
                CreateEmbedFooter::new(format!("Owned by {}", &user.name)).icon_url(user.face()),
            );
        }
        Ok(emb)
    }
