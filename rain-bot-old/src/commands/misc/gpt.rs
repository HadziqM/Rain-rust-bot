use crate::{MyErr,SlashBundle,ComponentBundle,ModalBundle,Mytrait,Components,Mybundle};
use serenity::all::*;
use crate::reusable::gpt::{Gpt,chat::CompModel};
...
async fn button(bnd:&ComponentBundle<'_>)->Result<(),MyErr>{
    ...
    while let Some(x) = msg.next().await {
        let z = ModalBundle{cmd:&x,ctx:bnd.ctx(),image:bnd.image,init:bnd.init,pedia:bnd.pedia};
        modal(&z).await?;
        break;
    }
...
fn modal_register_row(name:&str)->CreateActionRow{
    ...
    CreateActionRow::InputText(
        CreateInputText::new(InputTextStyle::Paragraph, name, name).required(true).placeholder(&placeholder)
    )
}
fn modal_response(id:&str)->CreateInteractionResponse{
    let name = format!("chat-{id}");
    CreateInteractionResponse::Modal(CreateModal::new(name, "Your Chat Response")
