...
use crate::{material::ItemPedia, Components, MyErr, Mybundle, Mytrait, SlashBundle};
use serenity::all::*;
...
async fn slash(bnd: &SlashBundle<'_>) -> Result<(), MyErr> {
    for i in &bnd.cmd.data.options {
        if let CommandDataOptionValue::String(x) = &i.value {
            ...
            let reverse = [data[2], data[3], data[0], data[1]]
                .iter()
                .collect::<String>();
            let auto = format!(
                "https://xl3lackout.github.io/MHFZ-Ferias-English-Project/sozai/sozai.htm?{}",
                &reverse
            );
            let y = CreateInteractionResponse::Message(
                CreateInteractionResponseMessage::new().components(vec![CreateActionRow::Buttons(
                    vec![CreateButton::new_link(&auto).label(ItemPedia::search(7, x).unwrap())],
                )]),
            );
