...
async fn slash(bnd: &SlashBundle<'_>, reg: &Reg<'_>) -> Result<(), MyErr> {
    ...
    if !multi {
        ...
        g_pg = result.0;
        raw = image
            .multi_pull(result.1.clone(), bnd.pedia, bnd.image)
            .await?;
        g_data = result.1;
    }
    let code: Vec<_> = g_data.iter().map(|e| e.code.to_owned()).collect();
    reg.pg
        .send_distrib(&g_pg, &code, reg.cid, bnd.pedia)
        .await?;
