use crate::reusable::component::bounty::{
    Bounty, BountyRefresh, BountySubmit, BountyTitle, Category, Methode, BBQ,
};
use crate::reusable::component::shutdown::Shutdown;
use crate::{ComponentBundle, Components, MyErr, Mybundle, Mytrait, PgConn, SlashBundle};
use serenity::all::*;
use serenity::futures::StreamExt;
use std::time::Duration;

fn select_menu(id: &str, op: Vec<(String, String)>) -> CreateSelectMenu {
    let options: Vec<_> = op
        .iter()
        .map(|x| CreateSelectMenuOption::new(&x.1, &x.0))
        .collect();
...
fn make_arow(menu: Vec<CreateSelectMenu>) -> Vec<CreateActionRow> {
    menu.iter()
        .map(|x| CreateActionRow::SelectMenu(x.to_owned()))
        .collect()
}
async fn msg_edit(
    msg: &mut Message,
    arow: Vec<CreateActionRow>,
    ctx: &Context,
) -> Result<(), MyErr> {
    msg.edit(&ctx.http, EditMessage::new().components(arow))
        .await?;
    Ok(())
}
...
async fn slash(bnd: &SlashBundle<'_>) -> Result<(), MyErr> {
    ...
    let link = &msg
        .attachments
        .first()
        .ok_or(MyErr::Custom("cant get your msg attachment".to_owned()))?
        .url;
    ...
    bnd.cmd
        .create_response(
            &bnd.ctx.http,
            CreateInteractionResponse::Message(CreateInteractionResponseMessage::new().components(
                make_arow(vec![m_menu.clone(), c_menu.clone(), b_menu.clone()]),
            )),
        )
        .await?;
    let mut msg2 = bnd.cmd.get_msg(bnd.ctx).await?;
    let mut rply = msg2
        .await_component_interactions(&bnd.ctx)
    ...
    while let Some(pat) = rply.next().await {
        if pat.user != bnd.cmd.user {
            pat.response(
                bnd.ctx,
                Components::interaction_response("the menu isnt for you", true),
            )
            .await?;
        }
    ...
    if !submit.cooldown(&mut cooldown).await? {
        return Err(MyErr::Custom(
            "The Bounty You selected is on cooldown or disabled".to_owned(),
        ));
    }
    ChannelId::new(bnd.init.bounty.judge_ch)
        .send_message(
            &bnd.ctx.http,
            CreateMessage::new()
                .embed(submit.embed())
        ...
        .await?;
    msg.reply(&bnd.ctx.http, "Your bounty is already submitted to Judge")
        .await?;
...
pub(super) async fn submit(bnd: &SlashBundle<'_>) -> Result<(), MyErr> {
    ...
    let url = &bnd
        ...
        .values()
        .next()
        .ok_or(MyErr::Custom("Cant get attachment".to_owned()))?
        .url;
    let mut mention = "";
    let mut methode = "";
    let mut category = "";
    ...
    let submit = BountySubmit::new(
        ...
        false,
        member,
        Components::get_mentions(mention),
        &mut pg,
        &bounty,
        &url,
        met,
...
async fn distrib(bnd: &SlashBundle<'_>) -> Result<(), MyErr> {
    ...
    let mut pg = PgConn::create(bnd.init, member.user.id.to_string()).await?;
    let bounty = Box::new(Bounty::new(&cat).await?);
    let mut submit = BountySubmit::new(
        bnd,
        true,
        member,
        Components::get_mentions(mention),
...
async fn button(bnd: &ComponentBundle<'_>) -> Result<(), MyErr> {
    ...
    let user = code
        .next()
        .ok_or(MyErr::Custom("cant get user id in custom id".to_owned()))?;
    let state = code.next().ok_or(MyErr::Custom(
        "cant get the button state in custom id".to_owned(),
    ))?;
    let mut submit = BountySubmit::open(user).await.ok_or(MyErr::Custom(
        "submit data doesnt exist on cache".to_owned(),
    ))?;
    ...
    if state == "r" {
        ...
        ch.send_message(
            ...
            CreateMessage::new().content(format!(
                "<@{}> bounty is rejected by {}",
                user, &bnd.cmd.user.name
            )),
        )
        .await?;
    ...
    ChannelId::new(bnd.init.bounty.receptionist_ch)
        .send_message(
            &bnd.ctx.http,
            CreateMessage::new().content(format!(
                "{} submitted bounty accepted by {}",
                submit.hunter[0].member.user.to_string(),
                bnd.cmd.user.name
            )),
        )
        .await?;
    ...
    ChannelId::new(bnd.init.bounty.conquered_ch)
        .send_message(&bnd.ctx.http, CreateMessage::new().embed(submit.embed()))
        .await?;
    submit.delete().await;
    msg.delete(&bnd.ctx.http).await?;
    pg.close().await;
