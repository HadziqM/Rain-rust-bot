...
use crate::reusable::component::bounty::{Category, Methode, BBQ};
use crate::AppReg;
use serenity::all::*;
...
pub fn reg() -> Vec<CreateCommand> {
    ...
    let dist = AppReg::admin_slash("distribution", "distribute bounty reward to player")
        ...
        .add_option(AppReg::str_option(
            "mentions",
            "mention all user (can be more than 1) to send",
        ));
    let bounty = AppReg::normal_slash("bounty", "command group for bounty")
        .add_option(
            AppReg::subcommand("submit", "submit your bounty useing second methode")
                .add_sub_option(
                    AppReg::att_option("image", "the image proof for your bounty").required(true),
                )
                .add_sub_option(methode.clone())
                .add_sub_option(category.clone())
                .add_sub_option(bbq.clone())
                .add_sub_option(AppReg::str_option(
                    "mentions",
                    "your team on bounty if multiplier",
                )),
        )
        .add_option(
            AppReg::subcommand("pedia", "your bounty wikipedia")
                .add_sub_option(category.clone())
                .add_sub_option(bbq.clone()),
        );
    vec![AppReg::message_context("🎮 Submit"), bounty, dist, cooldown]
}
