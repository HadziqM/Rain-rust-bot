...
use crate::{Components, ItemPedia, MyErr, Mybundle, Mytrait, Reg, SlashBundle};
...
impl MarketHandle {
    fn new(cmd: &CommandInteraction) -> Result<MarketHandle, MyErr> {
        let err =
            MyErr::Custom("invalid input, please check your input especially on unit".to_string());
    ...
    fn code(&self) -> ItemCode {
        ItemCode {
            key: self.data.value().to_owned(),
            count: self.count,
            types: self.data.code(),
        }
    }
}
impl SubCommand {
...
async fn auto(bnd: &SlashBundle<'_>) -> Result<(), MyErr> {
    ...
    let content = CreateInteractionResponse::Autocomplete(
        CreateAutocompleteResponse::new().set_choices(choice),
    );
...
pub async fn slash(bnd: &SlashBundle<'_>) -> Result<(), MyErr> {
    ...
    reg.pg
        .market(&option.code(), reg.cid, option.price, bnd.pedia)
        .await?;
