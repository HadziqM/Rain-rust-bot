...
impl Market {
    ...
    pub async fn new() -> Result<Market, MyErr> {
        Ok(serde_json::from_str(
            &tokio::fs::read_to_string(&Market::path()).await?,
        )?)
    }
                ...
                if name.to_lowercase().contains(&foc) || name.to_lowercase().starts_with(&foc) {
                    if y.treshold != 0 {
                        return Some(AutocompleteChoice {
                            value: Value::String(y.item.key.to_owned()),
                            name,
                        });
                    }
                }
    ...
    pub fn item_hash(&self) -> HashMap<String, Item> {
        self.market
            .iter()
            .map(|y| (y.item.key.to_owned(), y.clone()))
            .collect()
    }
    ...
    pub async fn send(
        ...
        if init.server_role.admin_role == msg.author.id.get() {
            let mid = msg
                ...
                .send_message(
                    &ctx.http,
                    CreateMessage::new().embed(self.make_embed(pedia)),
                )
                .await?;
        ...
        Err(MyErr::Custom(
            "You Cant youse admin only Command".to_string(),
        ))
    }
    pub async fn update(&self, ctx: &Context, init: &Init, pedia: &ItemPedia) -> Result<(), MyErr> {
        let mut msg = ChannelId(NonZeroU64::new(init.log_channel.market_channel).unwrap())
            .message(&ctx.http, init.log_channel.market_channel_msg)
            .await?;
        msg.edit(&ctx.http, EditMessage::new().embed(self.make_embed(pedia)))
            .await?;
        Ok(())
    }
    pub async fn update_new(ctx: &Context, init: &Init, pedia: &ItemPedia) -> Result<(), MyErr> {
        ...
        let mut msg = ChannelId(NonZeroU64::new(init.log_channel.market_channel).unwrap())
            .message(&ctx.http, init.log_channel.market_channel_msg)
            .await?;
        msg.edit(&ctx.http, EditMessage::new().embed(x.make_embed(pedia)))
            .await?;
        Ok(())
    }
...
impl Default for Tag {
    fn default() -> Self {
        Tag {
            tag: vec![TagItem::default(), TagItem::default()],
        }
    }
}
