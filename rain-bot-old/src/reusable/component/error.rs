...
impl<'a> ErrorLog<'a> {
    ...
    pub fn make_embed(&self, severity: bool) -> CreateEmbed {
        ...
        CreateEmbed::new()
            ...
            .footer(
                CreateEmbedFooter::new(format!("you can consult this to {}", self.user.tag()))
    ...
    pub async fn log_error_channel(&self, severity: bool) {
        ...
        if let Err(why) = ch_id
            .send_message(
                &self.ctx.http,
                CreateMessage::new()
                    .embed(self.make_embed(severity))
                    .add_file(self.att.to_owned())
                    .content(format!("for {}", self.usr.to_string())),
            )
            .await
        {
    ...
    pub fn defer_response(&self, severity: bool) -> EditInteractionResponse {
        EditInteractionResponse::new()
            .embed(self.make_embed(severity))
            .new_attachment(self.att.to_owned())
    }
}
