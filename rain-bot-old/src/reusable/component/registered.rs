...
use crate::{
    reusable::postgress::card::UserData, Components, Init, MyErr, Mybundle, Mytrait, PgConn,
};
...
impl<'a> Reg<'a> {
    async fn select_card<T: Mytrait>(
        &self,
        data: UserData,
        ctx: &'a Context,
        cmd: &'a T,
    ...
    pub async fn switch<T: Mytrait>(
        ctx: &'a Context,
        cmd: &'a T,
        init: &'a Init,
        bypass: bool,
        _ephemeral: bool,
    ) -> Result<Option<Reg<'a>>, MyErr> {
        let user = cmd.user();
    ...
    pub async fn reverse_check<T: Mybundle>(
        bnd: &'a T,
        user: &'a User,
    ) -> Result<Option<Reg<'a>>, MyErr> {
        let pg = PgConn::create(bnd.init(), user.id.to_string()).await?;
    ...
    pub async fn reverse_check_alter<T: Mybundle>(
        bnd: &'a T,
        user: &'a User,
    ) -> Result<Option<Reg<'a>>, MyErr> {
        let pg = PgConn::create(bnd.init(), user.id.to_string()).await?;
    ...
    pub async fn only_check_alter<T: Mybundle>(
        bnd: &'a T,
        user: &'a User,
    ) -> Result<Reg<'a>, MyErr> {
        let pg = PgConn::create(bnd.init(), user.id.to_string()).await?;
