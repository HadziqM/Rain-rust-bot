...
impl<'a> PgConn<'a> {
    ...
    pub async fn buy_ticket(&self, ticket: i32) -> Result<(), BitwiseError> {
        sqlx::query("update discord set ticket=ticket+$1 where discord_id=$2")
            .bind(ticket)
            .bind(&self.did)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn ticket_all(&self, ticket: i32) -> Result<(), BitwiseError> {
        sqlx::query("update discord set ticket=ticket+$1 where ticket is not null")
            .bind(ticket)
            .bind(&self.did)
            .execute(&self.pool)
            .await?;
...
mod testing {
    ...
    async fn test_bounty_send() {
        ...
        let mut pg = PgConn::create(&init, init.discord.author_id.to_string())
            .await
            .unwrap();
