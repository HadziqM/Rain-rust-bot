...
impl<'a> PgConn<'a> {
    ...
    async fn get_all_cid(&self, uid: i32) -> Result<Vec<i32>, BitwiseError> {
        let row = sqlx::query("SELECT id FROM characters WHERE user_id=$1")
            .bind(uid)
            .fetch_all(&self.pool)
            .await?;
    ...
    pub async fn get_user_data_long(&self) -> Result<UserData, BitwiseError> {
        ...
        if cid != 0 && rid == 0 {
            let uid: i64 = sqlx::query("SELECT user_id FROM characters WHERE id=$1")
                ...
                .await?
                .try_get("user_id")?;
            sqlx::query("INSERT INTO discord_register (discord_id,user_id) VALUES ($1,$2)")
                .bind(&self.did)
                .bind(uid as i32)
                .execute(&self.pool)
                .await?;
