use serenity::all::*;
use crate::{Init,MyErr,ComponentBundle,Mytrait,Mybundle,Components};
...
pub(super) async fn join(ctx:&Context,member:&Member,init:&Init)->Result<(),MyErr>{
    let arow = CreateActionRow::Buttons(vec![
        ...
        CreateButton::new_link("https://discord.com/channels/937230168223789066/952959980749881434").label("Server Guide")
    ]);
    let count = member.guild_id.to_partial_guild_with_counts(&ctx.http).await?.approximate_member_count.unwrap();
    ...
    ChannelId::new(init.log_channel.join_channel).send_message(&ctx.http,
        CreateMessage::new().embed(embed).components(vec![arow]).content(format!("{}",member.user.to_string()))).await?;
    ...
    member.user.direct_message(&ctx.http, CreateMessage::new().embeds(vec![rule1,rule2,rule3])).await?;
    Ok(())
}
