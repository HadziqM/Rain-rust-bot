...
async fn auto(bnd:&SlashBundle<'_>)->Result<(),MyErr>{
    ...
    for i in title.custom{
        let opt = AutocompleteChoice{name:format!("Trigger on {}",BountyTitle::name(&i.trigger)),value:Value::String(i.trigger)};
        option.push(opt);
    }
    option.push(AutocompleteChoice { name: format!("Trigger on {}",BountyTitle::name(&title.bronze_bounty.trigger)), value: Value::String(title.bronze_bounty.trigger) });
    option.push(AutocompleteChoice { name: format!("Trigger on {}",BountyTitle::name(&title.silver_bounty.trigger)), value: Value::String(title.silver_bounty.trigger) });
    option.push(AutocompleteChoice { name: format!("Trigger on {}",BountyTitle::name(&title.gold_bounty.trigger)), value: Value::String(title.gold_bounty.trigger) });
    option.push(AutocompleteChoice { name: format!("Trigger on {}",BountyTitle::name(&title.bronze_trading.trigger)), value: Value::String(title.bronze_trading.trigger) });
    option.push(AutocompleteChoice { name: format!("Trigger on {}",BountyTitle::name(&title.silver_trading.trigger)), value: Value::String(title.silver_trading.trigger) });
    option.push(AutocompleteChoice { name: format!("Trigger on {}",BountyTitle::name(&title.gold_trading.trigger)), value: Value::String(title.gold_trading.trigger) });
...
async fn test(bnd:&SlashBundle<'_>)->Result<(),MyErr>{
    ...
    let att = CreateAttachment::bytes(image.image.title(bnd.cmd.user.static_avatar_url().unwrap_or(bnd.cmd.user.default_avatar_url()).as_str()).await?, "title.png");
...
async fn gift(bnd:&SlashBundle<'_>)->Result<(),MyErr>{
    ...
    let mut mem = bnd.cmd.guild_id.unwrap().member(&bnd.ctx.http, &user.id).await?;
