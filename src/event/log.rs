...
use serenity::{prelude::Context, model::{prelude::{UserId, ChannelId}, user::User}, builder::CreateMessage};
...
pub async fn logging(ctx:&Context,init:&Init,wish:Option<User>){
    ...
    if let Err(why)=channel.send_message(&ctx.http,CreateMessage::new().add_file(&path).content(&format!("LOG AT <t:{}:F>"
    ...
    if let Some(wish)=wish{
        if let Err(why)= wish.dm(&ctx.http,CreateMessage::new()
                .content("server crash on about now or few minutes before, please check the log").add_file(&path)).await{
