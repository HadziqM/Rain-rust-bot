...
pub(super) async fn slash(bnd:&SlashBundle<'_>,reg:&Reg<'_>)->Result<(),MyErr>{
    ...
    if !trade.restourant.enabled{
        return Err(MyErr::Custom("restourant is currently disabled".to_string()));
    }
...
pub(super) async fn auto(bnd:&SlashBundle<'_>,focus:&str)->Result<(),MyErr>{
    let meal = Meal::new().await?;
    let mut auto = meal.meals.iter().filter_map(|x|{
        ...
        if name.starts_with(&foc) || name.contains(&foc){
            return Some(AutocompleteChoice{
                value:Value::Number(x.id.into()),name:x.name.to_owned()});
        }
        None
    }).collect::<Vec<_>>();
    if auto.len() > 25{
    ...
    bnd.cmd.create_response(&bnd.ctx.http, CreateInteractionResponse::Autocomplete(
