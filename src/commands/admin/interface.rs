...
async fn slash(bnd:&SlashBundle<'_>)->Result<(),MyErr>{
    ...
    if !bnd.init.mhfz_config.account_creation{
        button.push(
            Components::normal_button("Bind", "bind", ButtonStyle::Primary, "🎀")
        )
    }
