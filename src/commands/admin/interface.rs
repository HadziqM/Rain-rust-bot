...
use serenity::all::{ButtonStyle, RoleId, Colour};
...
async fn slash(bnd:&SlashBundle<'_>)->Result<(),MyErr>{
    ...
    let resp = CreateInteractionResponse::Message(CreateInteractionResponseMessage::new().embeds(vec![emb,emb2,emb3]).components(vec![arow,arow2]));
