...
impl Rectangle {
    ...
    pub(super) async fn get_rect(
        url: &str,
        radius: u32,
        off_x: u32,
        off_y: u32,
    ) -> Result<Rectangle, CustomImageError> {
        ...
        let read = image::io::Reader::new(Cursor::new(bytes))
            ...
            .decode()?;
        let img = read
            .resize_exact(radius * 2 + 1, radius * 2 + 1, FilterType::Nearest)
            .to_rgb8();
        Ok(Rectangle {
            radius,
            off_x,
...
impl GachaImage {
    ...
    async fn url_pull(
        ...
        pedia: &ItemPedia,
        imgg: &Images,
    ) -> Result<ImageBuffer<Rgb<u8>, Vec<u8>>, CustomImageError> {
        let bytes = gacha.result.raw(imgg);
        let mut img = image::io::Reader::new(Cursor::new(bytes))
            .with_guessed_format()?
            .decode()?
            .to_rgb8();
        ...
        let text = match gacha.code.text(pedia) {
            ...
            None => {
                return Err(CustomImageError::Custom(
                    "the key value on item code was false",
                ));
            }
        };
    ...
    pub async fn multi_pull(
        ...
        pedia: &ItemPedia,
        imgg: &Images,
    ) -> Result<Vec<u8>, CustomImageError> {
        ...
        for (x, y, px) in img.enumerate_pixels_mut() {
            ...
            *px = all_buff[index]
                .get_pixel(x_position as u32, y_position as u32)
                .to_owned()
        }
