...
use serenity::all::{CreateActionRow, ButtonStyle};
...
impl MyContext<'_> {
    ...
    pub async fn card_command(&self,user:Option<User>) -> Result<(),MyErr>{
        ...
        let usr;
        if let Some(us) = user {
            cid = self.data().user_reg(&us.id.to_string()).await.complete_only()?.2;
            usr = us;
        }else {
        ...
        self.send(CreateReply::default().embed(card_embed(&card,&usr))).await?;
        Ok(())
    }
    pub async fn registered_command(&self) -> Result<Option<(i32,String,i32)>,MyErr> {
        let (uid,uname,ucid) = self.data().user_reg(&self.author().id.to_string()).await.non_unreg()?;
        match ucid {
    ...
    pub async fn user_reg(&self, did:&str) -> UserReg {
        match self.db.get_user(did).await {
            Ok(data) => match self.db.get_char_id(did).await {
                Ok(cid) => UserReg::Complete { uid: data.0, uname: data.1, cid },
                Err(err) => UserReg::NoCharacter { uid: data.0, uname: data.1, err:err.into() }
            }
...
impl UserReg {
    pub fn unreg_only(&self) -> Result<(),MyErr> {
        ...
        Err(MyErr::from("You already registered, this command are only for unregistered"))
    }
    pub fn complete_only(&self) -> Result<(i32,String,i32),MyErr> {
        match self {
            Self::Complete { uid, uname, cid } => Ok((uid.to_owned(),uname.to_owned(),cid.to_owned())),
            Self::Unregistered { err } => Err(err.into()),
            Self::NoCharacter { uid:_, uname:_, err } => Err(err.into()),
        }
    }
    pub fn non_unreg(&self) -> Result<(i32,String,Option<i32>),MyErr> {
        match self {
            Self::Complete { uid, uname, cid } => Ok((uid.to_owned(),uname.to_owned(),Some(cid.to_owned()))),
            Self::Unregistered { err } => Err(err.into()),
            Self::NoCharacter { uid, uname, err:_ } => Ok((uid.to_owned(),uname.to_owned(),None)),
        }
    }
}
...
fn card_embed(card:&Card,user:&User) -> CreateEmbed {
        CreateEmbed::new()
        .title(card.name.as_str()).fields(vec![
        ("User",&format!("username: {}\nuser_id: {}\nchar_id: {}\nlast_login: {}",&card.username,card.user_id,card.char_id,card.last_login()),false),
        ("Character",&format!("HR: {}\nGR: {}",card.hrp(),card.gr),false),
        ("Guild",&format!("name: {}\nguild_id: {}",&card.g_name(),&card.g_id()),false)
    ]).footer(CreateEmbedFooter::new(format!("owned by {}",user.name)).icon_url(user.face()))
        .colour(MyColor::random()).thumbnail(&card.get_path())
}
...
async fn card_select(ctx:Context<'_>,user:&User,uid:i32) -> Result<(),MyErr> {
    ...
    while let Some(pat) = reply.next().await {
        ...
        if id=="next"{
            ...
            ctx.data().db.switch(card[index].char_id,&user.id.to_string()).await?;
            pat.create_response(ctx,Components::interaction_response("successfully switch main character", true)).await?;
            break;
        }
    }
