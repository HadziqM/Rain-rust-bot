use crate::{MyErr,Context, model::MyContext,ApplicationContext, utils::MyColor, setup::AppData};
use binding::postgres::account::AccountData;
use poise::{Modal, CreateReply};
use serenity::{all::{RoleId, ChannelId}, builder::{CreateEmbed, CreateMessage}};
...
pub async fn modal_invoke(ctx:ApplicationContext<'_>, reg:bool) -> Result<(),MyErr> {
    ...
    if let Some(data) = RegMod::execute(ctx).await? {
        let user = ctx.data().db.create_account(
            &data.username,
            &data.password,
            data.psn_id.clone(),
            reg,
        ...
        if !reg {
            ctx.say("account are partially binded, proceed to next step").await?;
        ...
        ctx.send(CreateReply::default().embed(reply.clone()).ephemeral(true)).await?;

        let channel = ChannelId::new(ctx.data.init.read().await.log_channel.account_channel);
        channel.send_message(ctx, CreateMessage::new().embed(reply)).await?;
    }
    Ok(())
}
...
#[poise::command(slash_command)]
async fn add_psn(ctx:Context<'_>,
    #[description = "your console psn id"]
    psn_id:String) -> Result<(),MyErr> {
